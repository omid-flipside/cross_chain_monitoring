...
# Single chain Analysis
    ...
    with c1:
        ...
        st.metric(label='**Average Transactions/Block**', value=df['Transactions/Block'].map('{:,.0f}'.format).values[0])
    with c2:
        st.metric(label='**Total Transactions**', value=str(df['Transactions'].map('{:,.0f}'.format).values[0]))
        st.metric(label='**Average TPS**', value=df['TPS'].map('{:,.2f}'.format).values[0])
    with c3:
